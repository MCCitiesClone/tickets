...
export function Embed({
  ...
  return (
    <div
      className="mt-1 grid max-w-lg gap-1 rounded-md border-l-4 bg-black/20 p-3 text-sm"
    ...
    >
      ...
      {embed.fields && embed.fields.length > 0 && (
        <div className="mt-1 flex flex-wrap gap-x-6 gap-y-2">
          {embed.fields.map((f, i) => (
            <div key={i} className={f.inline ? "min-w-[8rem] flex-1" : "w-full"}>
      ...
      {embed.image?.url && (
        ...
        <img
          src={embed.image.url}
          alt=""
          className="mt-1 max-h-72 rounded-md"
        />
      )}
      ...
      {embed.thumbnail?.url && !embed.image?.url && (
        ...
        <img
          ...
          alt=""
          className="mt-1 max-h-32 w-fit rounded-md"
        />
      )}
